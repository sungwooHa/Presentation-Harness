...
- **AI 안티패턴 검출 시 ERROR 등급**: 아래 요소가 발견되면 WARNING이 아닌 **ERROR**로 부여:
  - 네모 박스/카드 격자 (3개+ 동일 크기)
  - `border-left` 세로선 인용
  - `<hr>` / em dash 장식 구분선
  - 텍스트 화살표(`→`, `▸`) 플로우 나열
  - 이미지 배경에 오버레이 없이 텍스트 직접 배치
  검출 근거: "AI 생성물 전형 패턴"으로 사용자 피드백 2회+ 확인. (2026-05-14, CSR 최종발표회)
